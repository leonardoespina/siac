  ...
  <q-page class="report-page bg-white text-black">
    ...
    <div v-else-if="transaction" class="q-pa-lg bg-white text-black" style="max-width: 900px; margin: 0 auto;">
      ...
      <q-banner v-if="transaction.status === 'DRAFT'" class="bg-grey-3 text-grey-8 text-center text-h5 text-weight-bold q-mb-lg border rounded-borders">
        ...
        DOCUMENTO NO APROBADO (BORRADOR)
      </q-banner>

      <!-- HEADER -->
      <div class="text-center q-mb-lg">
        <div class="text-h4 text-weight-bold">
          <template v-if="transaction.type === 'LOSS'">ACTA DE MERMA DE MERCANCÍA</template>
          <template v-else-if="transaction.type === 'SUPPORT'">ACTA DE APOYO INSTITUCIONAL</template>
          <template v-else>ACTA DE CONSUMO INTERNO</template>
        </div>
      ...
      <div class="row justify-between q-mb-lg">
        <div class="col-6">
          <div><span class="text-weight-bold">ID de Transacción:</span> #{{ transaction.id }}</div>
          <div><span class="text-weight-bold">Fecha de Registro:</span> {{ new Date(transaction.createdAt).toLocaleString() }}</div>
          <div><span class="text-weight-bold">Comedor / Almacén:</span> {{ transaction.source?.name || 'Local' }}</div>
        </div>
        <div class="col-6 text-right">
          <div v-if="transaction.type === 'SUPPORT'"><span class="text-weight-bold">Institución Receptora:</span> {{ transaction.institution?.name || 'No especificada' }}</div>
          <div><span class="text-weight-bold">Estado:</span> {{ transaction.status }}</div>
      ...
      <div class="text-subtitle1 text-weight-bold q-mb-sm">DETALLE DE MERCANCÍA</div>
      ...
      <q-markup-table flat bordered separator="cell" class="q-mb-lg bg-white text-black">
        <thead class="bg-grey-2">
          <tr>
            ...
            <th class="text-center">Cant. Registrada</th>
          </tr>
        </thead>
        <tbody>
      ...
      <div class="row q-mb-xl">
        <div class="col-12 col-sm-6">
          ...
          <div class="q-pl-md">
            <div v-for="total in totalsByUnit" :key="total"><span class="text-weight-bold">Total Registrado:</span> {{ total }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="q-mt-xl q-pt-xl">
        <div class="text-center q-mb-xl text-caption text-grey-8">
          Con las firmas expuestas a continuación, se da fe de la veracidad de la información reflejada en el presente documento.
        </div>
        
        <div class="row justify-between text-center q-mt-xl q-pt-xl">
          <div class="col-6 q-px-xl">
            <q-separator color="black" class="q-mb-sm" />
            <div class="text-weight-bold">Despachado / Declarado Por</div>
            <div class="text-caption">{{ transaction.createdBy?.name || 'Nombre, Cédula y Firma' }}</div>
          </div>
          <div class="col-6 q-px-xl">
            <q-separator color="black" class="q-mb-sm" />
            <div class="text-weight-bold">{{ transaction.type === 'SUPPORT' ? 'Recibido Por (Institución)' : 'Autorizado / Validado Por' }}</div>
...
import { useConsumptionPdfMake } from '~/composables/features/useConsumptionPdfMake'
...
const {
  ...
  totalsByUnit,
  closeTab
} = useTransferReport() // Reutilizamos este composable porque busca en /api/transfers/[id] que es el mismo endpoint

const { downloadPdf, isGenerating } = useConsumptionPdfMake()
