  ...
  <q-page padding>
    <div class="row items-center justify-between q-mb-md">
      <div class="text-h4 text-weight-bold text-primary">
        <q-icon name="history" class="q-mr-sm" /> Mi Historial de Turnos y Consumos
      </div>
    </div>
    ...
    <q-card flat bordered class="q-mb-md">
      <q-card-section class="row q-col-gutter-md items-center">
        ...
        <div class="col-12 col-md-5 row justify-end" :class="{'col-md-9': auth.user?.warehouseId}">
          ...
          <template v-slot:body="props">
            <q-tr :props="props" :class="props.row.status === 'OPEN' ? 'bg-blue-1' : ''">
              ...
              <q-td key="startTime" :props="props">
                <div>{{ new Date(props.row.startTime).toLocaleTimeString() }}</div>
                <div class="text-caption text-grey">{{ new Date(props.row.startTime).toLocaleDateString() }}</div>
              </q-td>
              <q-td key="endTime" :props="props">
                <div v-if="props.row.endTime">
                  <div>{{ new Date(props.row.endTime).toLocaleTimeString() }}</div>
                  <div class="text-caption text-grey">{{ new Date(props.row.endTime).toLocaleDateString() }}</div>
                </div>
                <div v-else>-</div>
              </q-td>
              <q-td key="shiftType" :props="props">
...
onMounted(() => {
  // Forzar filtro local si el usuario está atado a un comedor
  if (auth.user?.warehouseId) {
    filterWarehouse.value = auth.user.warehouseId
  }
})
</script>
